...
public class OpenAiListVocabularyService implements VocabularyService {
    ...
    public VocabularyTerm getDailyVocabulary(List<String> blacklist) {
        try {
            ...
            List<LlmVocabularyTerm> filteredTerms = llmGeneratedVocabularyList.items()
                                                                              .stream()
                                                                              .filter(term -> !blacklist.contains(term.word()))
                                                                              .toList();

            if (filteredTerms.isEmpty()) {
                throw new LlmNotAvailableException("All vocabulary terms are blacklisted");
            }

            // Pick a random term from the list
            LlmVocabularyTerm randomTerm = filteredTerms.get(random.nextInt(filteredTerms.size()));

            log.debug("Llm response: {} terms, selected: {}", llmGeneratedVocabularyList.items()
                                                                                        .size(), randomTerm);
            return new VocabularyTerm(sourceDocument, randomTerm.word(), randomTerm.definition(), randomTerm.partOfSpeech(),
                    randomTerm.pronunciation(), randomTerm.exampleSentence(), randomTerm.collocations(), randomTerm.synonyms());
    ...
    private QuestionAnswerAdvisor qaAdvisor(String filename) {
        ...
        return questionAwserAdvisorFactory.createAdvisorWithQuerySimilarityThresholdTopKAndFilterExpression(
                "Get the list of items from the vocabulary bank", .1, 10, filterExpression);
